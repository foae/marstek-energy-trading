...
- Reserve grid slices as late as possible to reach 100% by the next cheap-window deadline, recalculating from actual SOC with zero future solar forecast. Slices priced at most `CHARGE_DEFER_TOLERANCE_EUR_PER_KWH` above the cheapest allocation's average are affordable and are taken latest-first, so solar captured in the meantime shrinks the purchase; any remaining shortfall comes from the cheapest of the other slices. The earliest deferred piece starts one minute early, since the control loop ticks once a minute and is not aligned to it. Deferral is active only when a P1 meter is configured; without one SOC cannot rise while waiting, so the cheapest slices are reserved as before, under the same running-slice rule. Size reservations and plans at `CHARGE_POWER_W` times `CHARGE_PLANNING_DERATE`, not nameplate. An inventory sale must beat the no-sale plan by `INVENTORY_SALE_MIN_GAIN_EUR`. Exclude every grid slice that would individually violate the configured expected-profit floor.
...
- While charging, run the selection contiguously forward from now: the slice containing now is kept when it is within `CHARGE_DEFER_TOLERANCE_EUR_PER_KWH` of the cheapest average or when finishing its remaining energy costs under one cent (`chargeContinuationToleranceEUR`), and rising SOC shrinks the tail rather than the head, so falling prices cannot force a stop/start at every boundary. A dear slot merely entered at a tariff boundary costs far more than a cent and is used only if the shortfall needs it.
