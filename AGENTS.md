...
- While charging, run the selection contiguously forward from now: the slice containing now is kept whatever its price, and rising SOC shrinks the tail rather than the head, so falling prices cannot force a stop/start at every boundary.
