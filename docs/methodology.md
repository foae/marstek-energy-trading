...
The analyzer sizes grid purchases from the remaining DC shortfall to 100% divided by `BATTERY_CHARGE_EFFICIENCY`; initial stored energy therefore reduces the first grid purchase. A full discharge covers usable DC capacity times `BATTERY_EFFICIENCY` divided by `BATTERY_CHARGE_EFFICIENCY`. Both are AC-side energies divided by configured power to obtain duration. Execution recomputes the input needed from current SOC, sizes it at `CHARGE_POWER_W` times `CHARGE_PLANNING_DERATE`, and reserves the latest executable 15-minute slices priced within `CHARGE_DEFER_TOLERANCE_EUR_PER_KWH` of the cheapest allocation's average before the charge deadline, so solar captured in the meantime shrinks the purchase; any remaining shortfall is taken from the cheapest of the other slices, and assumed delivery capacity falls when measured charging power tapers. Deferral applies only when a P1 meter is configured, since without one no solar can arrive while waiting; that installation reserves the cheapest slices as before. Every selected grid slice must preserve the paired cycle's `MIN_PRICE_SPREAD` floor.
